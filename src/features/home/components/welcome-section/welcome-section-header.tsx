...
export function WelcomeSectionHeader({
    ...
    const displayTitle = userName
        ? (() => {
              try {
                  return `${title}, ${getUserFirstName(userName)}! 👋`;
              } catch {
                  return `${title}! 👋`;
              }
          })()
    ...
    return (
        <CardHeader>
            <CardTitle className={WELCOME_SECTION_STYLES.header.title}>{displayTitle}</CardTitle>
            <p className={WELCOME_SECTION_STYLES.header.subtitle}>{subtitle}</p>
        </CardHeader>
    );
}
