...
export function MatchesGridContainer({ containerVariants, children }: MatchesGridContainerProps) {
    const { grid } = MATCHES_GRID_CONFIG;
    const gridClassName = `grid ${grid.gap} ${grid.columns.tablet} ${grid.columns.desktop}`;

    return (
        <motion.div variants={containerVariants} className={gridClassName}>
            {children}
        </motion.div>
    );
}
